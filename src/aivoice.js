const libs = {
  Recorder: class Recorder {
    constructor(apiKey) {
      ...
      (this.language = "en"), (this.recordingColor = "antiquewhite");
    }
  ...
  writeText: function writeText(targetElement, text) {
...
let view = {
  ...
  createButton() {
    ...
    if (whisperButton) {
      // Button exists, log its presence
      return;
      console.log('Button with ID "whisper_voice_button" exists!');
    }
    ...
    button.addEventListener("pointerdown", async (event) => {
      ...
      this.elem.currentInputElem.scrollTo(100000, 1000000);
    });
  ...
  monitorFocus() {
    let focusHandler = (element) => {
      this.elem.currentInputElem = element;
      let targetElement = element;
      const rect = targetElement.getBoundingClientRect();
      // The rect object contains the position information
      const x = rect.left + rect.width * 0.85; // X position relative to the window
      const y = rect.top;
      console.log("Element in focus:", element);
      this.elem.voiceButton.style.left = x + "px";
      this.elem.voiceButton.style.top = y + "px";
    };
